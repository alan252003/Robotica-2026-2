...
class DynamicsManipulatorA():
  def __init__(self):
    pass
  def define_kinematics(self, kinematics:KinematicsManipulatorA):
  ...
  def define_dynamics(self, mass = [0.25, 0.25, 0.25]):
    ...
    self.m3 = mass[2]
    # Matrices de inercia
    self.Ic1 = self.inertia_tensor(self.kinematics.l1, 0.02, 0.01, self.m1)
    self.Ic2 = self.inertia_tensor(self.kinematics.l2, 0.02, 0.01, self.m2)
    self.Ic3 = self.inertia_tensor(self.kinematics.l3, 0.02, 0.01, self.m3)
    #Gravedad
    self.g = -9.81
  ...
  def effort_graph(self):
    fig, ((tau_1_g, tau_2_g, tau_3_g)) = plt.subplots(nrows=1, ncols = 3, figsize=(15,4))
    ...
    tau_3_g.plot(self.kinematics.t_m.T, self.tau_m[2, :].T, 'b-', linewidth=2)
    tau_3_g.set_xlabel("Tiempo (s)")
    tau_3_g.set_ylabel("Par (N·m)")
    tau_3_g.grid(True)
