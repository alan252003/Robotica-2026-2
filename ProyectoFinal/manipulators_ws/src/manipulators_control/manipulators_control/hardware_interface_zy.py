...
class HardwareInterfaceZY(Node):
  def __init__(self):
    super().__init__("hardware_interface_zy")
    ...
    self.joint_hardware_objectives_subscriber = self.create_subscription(
      JointState, "/zy_joint_hardware_objectives", self.hardware_obj_callback, 10
    )
    ...
    self.current_joint_state.name = ["base_column_joint", "column_arm_joint"]
    self.current_joint_state.position = [0.05, 0.05]
  ...
  def joint_states_timer_callback(self):
    ...
    msg.name = ["base_column_joint", "column_arm_joint"]
